...
const insertMR = async (payload) => {
    // try {
    if (!isArray(payload)) {
        payload = [payload];
    }

    const district = String(payload[0].districts).replace(/\\n/g, '').trim().split('_').join(' ');
    const organisations = await pullOrganisationUnits(3, district);
    ...
    if (district && district.length === 1) {
        ...
        let final = payload.map(pay => {
            let { list, _version, ...rest } = pay;

            // let { subcounty, date_of_results, districts, region, other_subcountyvisited } = rest;

            subcounty = String(subcounty).replace(/\\n/g, '').trim().split('_').join(' ');
            if (subcounty === 'OTHERS') {
                subcounty = String(other_subcountyvisited).split('_').join(' ');
            }

            const searchedSubCounty = subCounties.filter(s => {
                return String(s.name).replace(/\\n/g, '').trim().toLowerCase().includes(String(subcounty).replace(/\\n/g, '').trim().toLowerCase());
            });
    ...
    const subCounties = groupBy(final, 'subcounty');
